...
async function launchGame() {                                                       //单击“启动游戏”按钮时执行
    const promise = new Promise(async (resolve, reject) => {
        ...
        if (gamePath == undefined) reject('游戏路径未设置！')
        if (!(api.checkFile(gamePath))) reject('游戏路径无效!')
        resolve(gamePath)
    })
    promise
        .then((value) => {                                                          //路径有效
...
async function initGamePath() {                                                     //初始化游戏路径
    ...
    if (newGamePath == '') return false                                                  //如果选择“Cancel”，直接退出函数体
...
topbar.close.addEventListener('click', function () { api.closeWin() })
topbar.minimize.addEventListener('click', function () { api.minimizeWin() })
